...
> **当前版本 v1.4.1** ·「技术打分 + 基本面避雷 + 人工规则提示」三层体检 +「低吸选股」（不追涨停）+「体检记录 + 回测」（用真实涨跌验证战法）+「成交量 + 主力/散户资金流」。

---

## ✨ 能做什么

一次 `stock_diagnosis(code)` 给出**三层体检**：

**① 技术打分（战法计算层）**
- 均线（MA5/MA24生命线/MA60季线）、均量线金叉死叉、K线形态（涨停/光头大阳/红三兵/跳空/十字星）、MACD/威廉指标、大盘多空环境、周线确认
- 按 100 条战法可量化规则打分 → 输出 **买入候选 / 持有 / 卖出 / 回避**

**② 基本面避雷（risk_check）**
- 自动查**亏损**（净利润为负）、**估值异常**（PE 负 / >200）、**ST 风险**
- **妖股顶部识别**：近10日涨停≥3次 / 20日涨≥50% / 天量≥4倍 → 把"爆炒亏损股"从买入候选里降级为"高风险观望"
...
- 每次体检把"结论+收盘价+成交量+主力/散户资金流+命中信号"存到本地 `data/diagnosis_log.csv`（不上传）
- 过几天 `backtest()` 自动拉实际涨跌，按结论方向（看多→涨为对/看空→跌为对）统计命中率
- 用真实数据反哺阈值优化，而非凭感觉调参

**⑥ 资金面（fund_flow_em）— 主力 vs 散户**
- 当日主力净流入 / 超大单 / 大单 / 中单 / 小单（散户≈中小单）
- 走东财 push2 域名（连通性优于历史接口 push2his，间歇可达，内置重试+绕过系统代理）；拿不到则留空，不影响体检主流程
