...
发布了新版本（如本次 v1.6.2 前复权修复）后，按你的安装方式拉取最新：

**插件市场安装的（方式一，推荐）** —— 在 Claude Code 里：

```
/plugin marketplace update a-stock-tactics
/plugin update a-stock-tactics@a-stock-tactics
```

重启 Claude Code 后即生效。

**手动放 skill 目录的（方式二）** —— 重新 curl 覆盖那一个文件即可：
...
> **Windows 用户**：把路径换成 `%USERPROFILE%\.claude\skills\a-stock-tactics\SKILL.md`。
