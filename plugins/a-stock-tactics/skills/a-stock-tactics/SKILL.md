...
> **V1.6.2（前复权修复，送转/分红股不再失真）：** `get_daily()` 新增**自动前复权**——mootdx 默认返回不复权行情，遇送转/分红除权日会产生价格断层，污染所有 rolling 均线，把强势股的正常回调误判为空头崩塌（曾把刚 10转5 的中微公司 688012 误判「卖出·空头7条」，前复权后实为站稳生命线的「持有」）。修复方案：检测相邻两日收盘比值超出涨跌停极限的除权缺口，将缺口前 OHLC 等比例复权到缺口后口径。自包含、不依赖 mootdx 的 `adjust='qfq'`（规避其 pandas 2.x `fillna(method=)` bug）。**影响所有依赖 get_daily 的均线/形态/指标判定**。
>
> **V1.6.1（统一报告输出模板）：** 流程 A 新增「单票体检报告输出模板」——向用户呈现时按**九段式**完整组织（基本信息/基本面/趋势+K线/游资量价/主力散户/技术指标/解禁/多空对辩/纪律→裁判结论），缺失数据照实标注，不省略字段。让每次体检输出格式统一专业。
>
...
def get_daily(code: str, n: int = 70) -> pd.DataFrame:
    ...
    ★前复权：mootdx 默认返回不复权行情，遇送转/分红除权日会产生价格断层，污染 rolling 均线。
    本函数自动检测除权缺口并前复权，避免把强势股的正常回调误判为空头崩塌（详见 FAQ）。"""
    code = normalize(code)
    ...
    # ── 自动检测除权缺口并前复权（送转/分红/配股）──────────────────
    # 相邻两日收盘比值若超出该股单日涨跌停极限（留 1% 余量），判定为除权日，
    # 把缺口之前的 OHLC 按比例复权到缺口之后的口径，消除断层导致的均线失真。
    # 自包含、不依赖 mootdx 的 adjust='qfq'，因此不受其 pandas 2.x fillna(method=) bug 影响。
    lim = 0.21 if code.startswith(("688", "300", "301")) else 0.11  # 合法单日波动达不到此阈值
    closes = df['close']
    for i in range(len(df) - 1, 0, -1):
        prev = closes.iloc[i - 1]
        if prev and prev > 0:
            r = closes.iloc[i] / prev
            if not (1 - lim < r < 1 + lim):           # 超出涨跌停极限 → 除权缺口
                for col in ('open', 'high', 'low', 'close'):
                    df.loc[:i - 1, col] = df.loc[:i - 1, col] * r
                closes = df['close']                   # 复权后刷新，继续往前扫（可能多次除权）
    # ───────────────────────────────────────────────────────────────
...
#### 📋 单票体检报告输出模板（向用户呈现时，请按此完整格式组织，勿省略字段）

> `stock_diagnosis` 返回的字典已含全部数据。向用户展示时**统一按下面九段式呈现**，让报告完整专业。缺失数据照实标注 `[数据缺失]`，不要跳过该段。建议另调 `tencent_quote([code])` 补全市值/换手率/量比等基本信息。

```
# 📋 {名称}({代码}) · 完整体检报告（v1.6.x）
> 数据截至 {体检日} 收盘 · ⚖️ 裁判（Claude）综合 9 位分析师

## 📌 一、基本信息
现价/涨跌幅/总市值/流通市值/换手率/量比/涨停跌停价/题材  ← 来自 tencent_quote + 同花顺热点reason

## 💰 二、基本面（基本面分析师）
净利润/营收/PE/PB/ROE/资产负债率/现金流净利比 + 估值参照系  ← d["基本面"]

## 📈 三、趋势（技术分析师 + 近12日K线表）
现价 vs MA5/MA10/生命线MA24/季线MA60 + 站位 + 偏离5日线 + 近12日K线  ← d["均线"] + get_daily

## 🎯 四、游资/量价（游资分析师）
量比20日/换手 + 连板语义 + 是否妖股  ← d["量价异动"]

## 💸 五、主力 vs 散户（资金分析师）
主力净/超大单/散户净 + 资金博弈四态  ← d["资金面"]（缺失则标注并建议网络好时复查）

## 📊 六、技术指标
MACD/威廉WR/RSI(按A股解读)/VWMA/MA50-200  ← d["指标"]

## 🔓 七、解禁（解禁分析师）
解禁压力评级  ← d["基本面"]["解禁压力"] / lockup_pressure

## ⚖️ 八、多空对辩（多空辩手）
🐂多头席 vs 🐻空头席 逐条列出  ← d["多空对辩"]

## 📋 九、纪律提示（纪律提示官）
人工经验铁律逐条  ← d["需人工判断"]

## ⚖️ 裁判综合结论：【{五档评级}】
综合九位分析师，给出结论 + 操作逻辑（强调非投资建议）+ 关键观察位
末尾固定附：⚠️ 技术+基本面机械体检，不构成投资建议。
```
...
### Q: 刚做过送转/分红的股票，体检结论会不会失真？（前复权，V1.6.2 修复）
A: 不会了。mootdx 默认返回**不复权**行情，遇除权日（送转/大比例分红/配股）会产生价格断层——比如中微公司 10转5 后，除权日名义"-34%"（科创板单日跌停只有 -20%，这是除权缺口不是真跌），未复权的 MA5/MA24/MA60 全是除权前的高价，拿来跟除权后的现价比，会把强势股的正常回调误判成「跌破生命线/季线、空头崩塌」，给出方向相反的"卖出"结论。**V1.6.2 起 `get_daily()` 自动前复权**：检测相邻两日收盘比值超出涨跌停极限的除权缺口，把缺口前的 OHLC 等比例复权到缺口后口径，均线不再被污染。注意：① 对送转精确，对纯现金分红是近似（误差极小，不影响短线均线判断）；② `vol` 未做复权（均量线是相对比较，影响有限）；③ 自包含，不依赖 mootdx 的 `adjust='qfq'`（其复权代码在 pandas 2.x 会报 `fillna(method=)` 错）。
